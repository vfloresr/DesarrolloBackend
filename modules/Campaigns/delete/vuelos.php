...
if($_POST['Field120'] != ''){//todas las solicitudes del forumlario wufo entel
    ...
    if( strpos($_POST['Field225'], "/")==false and strpos($_POST['Field225'], "-") ==false){//wufo
        $_POST['Field225']=substr($_POST['Field225'],0,4).'-'.substr($_POST['Field225'],4,2).'-'.substr($_POST['Field225'],6,2);
    }
    if( strpos($_POST['Field230'], "/")==false and strpos($_POST['Field230'], "-") ==false){//wufo
...
if(isset($data->id_transaccion) and $data->id_transaccion != ''){
...
}else if(isset($data->notifications)){
	if (!isset($data->entryPoint)){
		...
		$_POST['politicas_privacidad_c']= 1;
		if($data->notifications == true){ 
			$_POST['recibir_notificaciones_c']= 1;
		}else {
			$_POST['recibir_notificaciones_c']= 0;
		}
		$_POST['description']=$json;  
	}	
}
...
if (isset($_POST['campaign_id']) && !empty($_POST['campaign_id'])) {
		if ($_POST['campaign_id'] == 'b019ca1c-0ad6-b892-916c-56b1029a7c1a'){
			$sql =	"select count(*) as cantidad 
						...
						join leads lea on lea.id = ema.bean_id and lea.campaign_id = 'b019ca1c-0ad6-b892-916c-56b1029a7c1a'
						where ema.bean_module = 'Leads'";

					  $resultado = $db->query($sql);
					  $row = $db->fetchByAssoc($resultado);
					  $cantidad_email = $row['cantidad'];
					  
			if($cantidad_email > 0 ){
				$rep['respuesta']="Email ya existe";
