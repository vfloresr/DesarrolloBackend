...
if(isset($data->id_campana) and $data->id_campana != ''){
	if ($data->id_campana = ''){
			$_POST['ip_registro_c'] = $_SERVER['REMOTE_ADDR'];
			$_POST['description']  = $_SERVER['HTTP_REFERER'];
			$_POST['lead_source'] = 'web';
			$_POST['lead_source_description'] = 'hoteles backend';
			$_POST['campaign_id'] = '3e0d4ed8-905d-4dcd-644a-5707188c4046';
	}else{
...
if (isset($_POST['campaign_id']) && !empty($_POST['campaign_id'])) {
			...
			/*	// busqueda del id lead
				$sql =	"  SELECT lea.id AS lead
							FROM   email_addr_bean_rel ema JOIN email_addresses ema1 ON ema.email_address_id = ema1.id AND ema1.email_address = '".$_POST['email1']."'
