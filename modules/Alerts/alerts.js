...
$.post(url,{module:'Alerts',action:'add',name:name,description:description,url_redirect:url_redirect,is_read:is_read,target_module:target_module,type:type}).done(function(data){if(data.redirect){Alerts.prototype.refreshPeriod=-1;}}).fail(function(data){console.log(data);}).always(function(){Alerts.prototype.updateManager();});}
Alerts.prototype.updateManager=function(){var url='index.php?module=Alerts&action=get&to_pdf=1';$.ajax(url).done(function(data,textStatus,jqXHR){if($('form.form-signin').length>0){Alerts.prototype.refreshPeriod=-1;return;}
...
Alerts.prototype.refreshPeriod=60000;function AlertObj(){this.title='Alert';this.options={body:' ',url_redirect:null,target_module:null,type:'info'};}
$(document).ready(function(){var updateMissed=function(){Alerts.prototype.updateManager();if(Alerts.prototype.refreshPeriod!=-1){setTimeout(updateMissed,Alerts.prototype.refreshPeriod);}}
updateMissed();});