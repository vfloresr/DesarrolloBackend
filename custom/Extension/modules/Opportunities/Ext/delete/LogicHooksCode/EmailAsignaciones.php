...
class EmailAsignaciones{
	...
	function CustomOpportunitiesEmail($emailTo, $emailSubject, $emailBody,$id_oportunidad,$email_u,$first_name,$last_name){
		...
		if(@$mail->Send()){
			...
			$email->status='sent';
			$email->intent='pick';
			$email->from_addr = $defaults['name'].' <'.$defaults['email'].'>'; 
			$email->to_addrs= $first_name.' '.$last_name.' <'.$email_u.'>';
	...
	function OpportunitiesWorkFlow(&$bean, $event, $arguments){
		global $db, $sugar_config,$current_user;
		$timeDate = new TimeDate();
		//if ($bean->sales_stage=='CerradoExterno' && $bean->fetched_row['sales_stage']!=$bean->sales_stage){
		
			
	/*	if($bean->sales_stage == 'RecepcionSolicitud' and $bean->assigned_user_id != $current_user->id )
		{
			$bean->fecha_asignacion_c=$bean->date_modified;
			$bean->save();

		
		}*/
		//cambio estado    
		// if ($bean->sales_stage == 'cotizacion' || $bean->sales_stage == 'PrimeraLlamada' || $bean->sales_stage == 'EnvioCotizacion' || $bean->sales_stage =='Seguimiento1' || $bean->sales_stage == 'Seguimiento2' || $bean->sales_stage == 'AsignacionSolicitudSupervisor' || $bean->sales_stage == 'contactado' || $bean->sales_stage == 'negociacion' ){
			// if ($bean->estado_wf_c != ''){
				// $oportunidad = new Opportunity();
				// $oportunidad->retrieve($bean->id);
				// $oportunidad->estado_wf_c='cambio_estado';
				// $oportunidad->save();
			// }
		// }
		...
		//cerrado ganado 
		// $GLOBALS['log']->fatal("inicio ".$bean->sales_stage.' '.$bean->opportunity_type.' '.$bean->assigned_user_id);
		if ($bean->sales_stage == 'CerradoGanado' && $bean->opportunity_type == 'solicitud_web'){
			...
			//$GLOBALS['log']->fatal("termino ganado");
		}
		}
		if ($bean->sales_stage=='CerradoExterno'){
			...
			if(	$user2->sucursal_c =='EXTERNOS'){
				...
				while($row = $db->fetchByAssoc($res)){
					...
					//$GLOBALS['log']->fatal("<td><a href='".$row['url']."' target='_blank'>$texto_link</a></td>");
						// if($row['url']!=''){
							// $emailtemplate->body_html =$emailtemplate->body_html."	<a href='".$row['url']."' target='_blank' ><button style:'background-color:red;color:withe'>Cotizar</button></a>";
						// }else{''}
						// if($row['pdf_joomla']!=''){
							// $emailtemplate->body_html =$emailtemplate->body_html."	
						// <a href='".$row['pdf_joomla']."' target='_blank'>$texto_pdf</a>
						// ";
						// else{''}
						// $emailtemplate->body_html =$emailtemplate->body_html."
				...
				$user = new User();
				$user->retrieve($bean->assigned_user_id);
				$this->CustomOpportunitiesEmail(array($user->email1), $emailtemplate->subject, $emailtemplate->body_html,$bean->id,$user->email1,$user->first_name,$user->last_name);
