...
//*****************************************************************************************//
       ...
       if ($_POST['tipo'] == 'user_workflow' && $acceso_wf->disponible_sw_c == 1 && $acceso_wf->sucursal_c == 'WORKFLOW'){ 
				...
				}elseif ($oportunidad > 60){
					echo '<script>alert("El Ejecutivo '.$nombre.' tiene Asignada mas de 50 Solicitudes Sin Cerrar..!!Por Favor Debe Cerrar Algunas solicitudes pendientes...");</script>';
				}else{
						$asig		=	explode("|",$_POST['asignado_a']);
						$id_user	= $asig[0];
						$full_name	= $asig[1];				
