...
// $list_favoritos = $usuarios->get_full_list("users.last_name", "users_cstm.favorito_c = 1 and users.status='Active' ",true);
	...
	$id_user  = (isset($asig[0] )) ? $asig[0] :  '' ;
	$nombre   = (isset($asig[1] )) ? $asig[1] :  '' ;
	$cantidad = (isset($_POST['solicitudes_asig'])) ? count($_POST['solicitudes_asig']):  '' ;
    $tipo     = (isset($_POST['tipo'] )) ? $_POST['tipo'] :  '' ;
	
	//$id_oportunidad  =  (isset( $_POST['oportunidad_id'] )) ? $_POST['oportunidad_id'] :  '' ; 
    //$valor      	 =  (isset( $_POST['valor'] )) ? $_POST['valor'] :  '' ; 
    $asig_directo    =  (isset( $_POST['asig_directo'] )) ? $_POST['asig_directo'] :  '' ; 

/*echo "Oportunidad:".$id_oportunidad."<br>\n";
echo "id_agente:".$valor."<br>\n"; 
echo "asig:".$asig_directo."<br>\n"; */
...
//*****************************************************************************************/ 
	 ...
	 //creamos fecha hora de asignacion
       ...
       if ($tipo  == 'user_workflow' && $acceso_wf->disponible_sw_c == 1 && $acceso_wf->sucursal_c == 'WORKFLOW'){ 
			$cant_opor = 50;
				...
				if($tarea > 0){
				   echo '<script>alert("El Ejecutivo '.$nombre.' tiene Asignada Solicitudes..!!Por Favor Espere 30 min");</script>';		
		...
		}else if($asig_directo == 1 ){
						
					//echo "entroooo";	
					
					//echo "id_oportunidad:".$_POST['oportunidad_id']."<br>";
					//echo "id_ejecutivo:".$_POST['valor']."<br>";
					// die();
					$usuario= new User();
					$usuario->retrieve($_POST['valor']);
					$oportunidad = new Opportunity();
					$oportunidad->retrieve($_POST['oportunidad_id']);
					$user_time_format = $current_user->getPreference('timef');
					$current_user->setPreference('timef', 'H:i:s');
					$fecha = date($current_user->getPreference('datef').' '.$user_time_format, strtotime(date('Y-m-d H:i:s')));
					$oportunidad->date_assigned_c=$fecha;
					$oportunidad->assigned_user_id = $_POST['valor'];
					$oportunidad->sales_stage ='asignado';
					$oportunidad->id_agente_c =$_POST['valor'];
					...
					echo '<script>alert("La Asignaci\u00F3n se realizo Exitosamente....");</script>';					
							...
							$texto = "
							  <h2>Administrator, le ha asignado una Oportunidad.</h2>
								...
								if(@$mail->Send()){
									...
									$email->status='sent';
									$email->intent='pick';
									$email->from_addr = 'noticias@cocha.com';
									$email->to_addrs= "Turismo Cocha < noticias@cocha.com >";
									$email->description_html=$texto;
									$email->assigned_user_id=$current_user->id;
									$email->assigned_user_name=$current_user->user_name;
									$email->from_name=$current_user->full_name;
									$email->parent_name='noticias@cocha.com';
									$email->date_sent=TimeDate::getInstance()->getNow(true)->modify("+0 days")->asDb();
									$email->parent_type='Opportunity';
									$email->parent_id=$oportunidad->id;
									$email->from_addr_name=$current_user->full_name;
