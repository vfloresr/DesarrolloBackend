...
//variables
$camapana_web = '4061e35f-dbab-e5c1-221c-56548dc162bd';
...
$users = array(
 ...
 if (isset( $_POST['id_joomla']) && $_POST['id_joomla'] != ''){
			...
			$precio = $data[0]['pricing'][0]['mainPrice'];
...
$ip_registro_c            = (isset( $_SERVER['REMOTE_ADDR'] )&& $_SERVER['REMOTE_ADDR'] != '' ) ? $_SERVER['REMOTE_ADDR'] :  '' ;
$website            = (isset( $_SERVER['HTTP_REFERER'] )&& $_SERVER['HTTP_REFERER'] != '' ) ? $_SERVER['HTTP_REFERER'] :  '' ;
$url_producto        = (isset( $_POST['url_producto'] )&& $_POST['url_producto'] != '' ) ? $_POST['url_producto'] :  '' ;
$recibir_notificaciones_c    = (isset( $_POST['recibir_notificaciones_c'] )&& $_POST['recibir_notificaciones_c'] != '' ) ? $_POST['recibir_notificaciones_c'] :  '' ;
$politicas_privacidad_c      = (isset( $_POST['politicas_privacidad_c'] )&& $_POST['politicas_privacidad_c'] != '' ) ? $_POST['politicas_privacidad_c'] :  '' ;
...
$precio_joomla       = (isset( $_POST['monto_pack'] )&& $_POST['monto_pack'] != '' ) ? $_POST['monto_pack'] : $precio  ;
$url_joomla          = (isset( $_POST['url_pack'] )&& $_POST['url_pack'] != '' ) ? $_POST['url_pack'] :  '' ;
$nombre_joomla       = (isset( $_POST['nombre_pack'] )&& $_POST['nombre_pack'] != '' ) ? $_POST['nombre_pack'] :  'OTROS' ;
...
if (isset( $_POST['agente_new'] )&& $_POST['agente_new'] != '' ){
$asig	 =	explode("/",$_POST['agente_new']);
$agente_c =$asig[0];
$id_agente_c =$asig[1];
}else{
$id_agente_c ='';
...
$monto_dolares       =(isset( $_POST['monto_dolares'] )&& $_POST['monto_dolares'] != '' ) ? $_POST['monto_dolares'] :  $precio  ;
$monto_pesos         =(isset( $_POST['monto_pesos'] )&& $_POST['monto_pesos'] != '' ) ? $_POST['monto_pesos'] :  $precio ;
...
for($i=0;$i<11;$i++)
{
	if(isset($_POST['edad_nino-'.$i]) && $_POST['edad_nino-'.$i] != ''){
		if ($i== 0){
			$edad_nino= $_POST['edad_nino-'.$i];
		}else{
			$edad_nino= $edad_nino." - ".$_POST['edad_nino-'.$i];
		}
	}else{
		$edad_nino= '';
	}
...
if ($email1){
	...
	//este query es utilizado para verificar si el contacto existe
			 $query = "SELECT l.id 
						...
						join leads l on l.id = er.bean_id  and l.campaign_id = '".$camapana_web."'
	...
	//sino existe el contacto se crea
		...
		$leads->email1=$email1;
		$leads->ip_registro_c =$ip_registro_c;
		$leads->recibir_notificaciones_c = $recibir_notificaciones_c;
		$leads->politicas_privacidad_c = $politicas_privacidad_c; 
		$leads->website = $website;
		$leads->campaign_id=$camapana_web;
		$leads->modulo = 'leads';
...
/*llamo a la funcion call para realizar la conexion del webservice */
	...
	$solicitud->url_wufo=$mail1.'/'.$phone_home.'/'.trim($rut_c).'/'.$first_name.'/'.$last_name;
	$solicitud->numero_producto=$numero_producto;
	$solicitud->categoria_id=$categoria_id;
	$solicitud->categoria_nombre=$categoria_nombre;
	$solicitud->modulo = 'solicitud';
	$solicitud->session = $login_result->id;
 ...
 /*llamo a la funcion call para realizar la conexion del webservice */
		$solicitud->id = call("acceso_user",$parametros, $_SESSION['url']);
	...
	$query = "SELECT leads_opportunities_1opportunities_idb
		...
		if($row = $db->fetchByAssoc($res)){
			$Opportunitys->id =  $row['leads_opportunities_1opportunities_idb'];
				...
				$Opportunitys->modulo = 'Opportunitys';
				$Opportunitys->session = $login_result->id;
				$parametros = array(
					'datos' => $Opportunitys
				);	
				/*llamo a la funcion call para realizar la conexion del webservice */
				$Opportunitys->id = call("acceso_user",$parametros, $_SESSION['url']);
		}else{
	...
	if (empty($Opportunitys->id)){
		...
		$Opportunitys->id_agente_c = $id_agente_c;
		$Opportunitys->agente_c = $agente_c;
		...
		$lead_r = new Lead();
		$lead_r->id = $leads->id;
		$lead_r->load_relationship('leads_opportunities_1');
		$lead_r->leads_opportunities_1->add($Opportunitys->id);
	}
	
	//relacion oportunidad solicitudes
	$oportunidad = new Opportunity();
	$oportunidad->id = $Opportunitys->id;
	$oportunidad->load_relationship('opportunities_crm_solicitudes_1');
    $oportunidad->opportunities_crm_solicitudes_1->add($solicitud->id);
	...
	echo "Error,no se guardo la solcitud porque debe enviar email";
}
