...
{	 
	...
	case "2" :			
						$query = " SELECT   u.id
						 , concat(u.last_name, ' ', u.first_name) AS usuario
						 , CASE WHEN ud.sucursal_c = 'EXTERNOS' THEN 'Ext' ELSE CASE WHEN ud.sucursal_c = 'WORKFLOW' THEN 'WF' ELSE 'no' END END AS tipo
						 , CASE WHEN ud.sucursal_c = 'EXTERNOS' THEN count(o.id) ELSE 0 END AS cantidad_ext
						 , SUM(CASE WHEN DATE(date_assigned_c) =  date(now()) THEN 1 ELSE 0 END) AS cantidad
						 , SUM(CASE WHEN DATE(date_assigned_c) <= date(now()) AND o.sales_stage IN ('recepcionado', 'asignado') THEN 1 ELSE 0 END) AS nuevas
				FROM     users u
						 JOIN users_cstm ud ON u.id = ud.id_c AND ud.sucursal_c not in ('','BACKEND') AND ud.sucursal_c = 'EXTERNOS'
	...
	case "3" :
		$query = "SELECT   u.id
						 ...
						 , SUM(CASE WHEN DATE(date_assigned_c) <= date(now()) AND o.sales_stage IN ('recepcionado', 'asignado') THEN 1 ELSE 0 END) AS nuevas
				FROM     users u
						 JOIN users_cstm ud ON u.id = ud.id_c AND ud.favorito_c = 1 AND ud.sucursal_c = 'EXTERNOS'
						 LEFT JOIN opportunities o ON o.assigned_user_id = ud.id_c
						 LEFT JOIN opportunities_cstm opc ON opc.id_c = o.id AND o.deleted = 0
				WHERE    u.status = 'Active'
				GROUP BY u.first_name, u.last_name
     ...
     case "4" :
		$query = " SELECT u.id,IF(ud.disponible_sw_c=1,'ACTIVO','INACTIVO') AS estado, concat(u.last_name,' ',u.first_name) usuario, ud.sucursal_c,
				  SUM(CASE WHEN   DATE(date_assigned_c) =  date(now())  THEN 1 ELSE 0 END) AS cantidad,
				  SUM(CASE WHEN  DATE(date_assigned_c) <= date(now())  AND o.sales_stage in ('recepcionado','asignado') THEN 1 ELSE 0 END) AS nuevas
				FROM users u
				JOIN  users_cstm ud       ON u.id = ud.id_c 
				LEFT JOIN opportunities o ON o.assigned_user_id = ud.id_c AND o.opportunity_type = 'solicitud_web' AND o.deleted = 0
				LEFT JOIN opportunities_cstm opc ON opc.id_c =o.id and o.deleted=0 
				WHERE u.status='Active'
				AND u.deleted = 0 
