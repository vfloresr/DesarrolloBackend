...
//$id_agente_c            = (isset( $_POST['id_agente_c'] )&& $_POST['id_agente_c'] != '' ) ? $_POST['id_agente_c'] :  '' ;
...
if ($email1){
	...
	//leads
	 ...
	 //es usado para validar por rut, victor solicito que sea por email.21/10/2015
			// if (isset($_POST['rut']) && $_POST['rut'] != '' ){
				// $query = "select id from leads where deleted = 0 and campaign_id = '4061e35f-dbab-e5c1-221c-56548dc162bd' and phone_fax = '".trim($rut_c)."'";
	...
	//este query es utilizado para verificar si el contacto existe
		//if (empty($leads->id) || $leads->id == ''){
			 $query = "SELECT l.id 
						...
						join leads l on l.id = er.bean_id and l.deleted = 0 and l.campaign_id = '".$camapana_web."'
	...
	$solicitud->url_wufo=$email1.'/'.$phone_home.'/'.trim($rut_c).'/'.$first_name.'/'.$last_name;
