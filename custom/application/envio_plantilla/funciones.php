<?php
 ...
 function nombre_ejecutivo ($id,$fecha){
 ...
 function mostrar_cumpleanio($id,$fecha){
global $db, $sugar_config;
  $cumpleanios = '';
 ...
 /* $query_cumpleanio = "SELECT id,
	 ...
	 $query_cumpleanio="SELECT 
				   ...
				   FROM contacts 
				   WHERE birthdate is not null
           AND SUBSTRING(birthdate FROM 6 FOR 10) = Date_format('".$fecha."','%m-%d')
           AND assigned_user_id = '".$id."' ORDER BY 1,2"; 			   
  ...
  function oportunidades_venta_cross($id,$fecha){
	global $db, $sugar_config;
    ...
    $query_cross = "SELECT
            		...
            		FROM
						opportunities o
							...
							JOIN crm_negocios_opportunities_c rn ON rn.crm_negocios_opportunitiesopportunities_idb = c.id_c
							JOIN crm_negocios n ON n.id = rn.crm_negocios_opportunitiescrm_negocios_ida
							JOIN crm_negocios_crm_pasajeros_c rp ON rp.crm_negocios_crm_pasajeroscrm_negocios_ida = n.id 
							JOIN crm_pasajeros p ON p.id = rp.crm_negocios_crm_pasajeroscrm_pasajeros_idb AND p.tipo_pax LIKE '%Comprador%'
							JOIN contacts co ON co.id = p.contact_id_c
							LEFT JOIN crm_negocios_cstm dn ON ((dn.id_c = n.id)) 
							WHERE o.opportunity_type in ('crosselling') 
							 AND date(o.date_entered) >= '".$fecha."'
    ...
    function oportunidades_venta_recompra($id,$fecha){
	global $db, $sugar_config;
                                                   ...
                                                   <tbody>
                                                       <tr>
                                                         <td valign="top" class="mcnTextContent" style="padding-top:9px; padding-right: 18px; padding-bottom: 9px; padding-left: 18px;">
                                                            ...
                                                            <span style="color: #006699;">Clientes que compraron contigo hace un a&ntilde;o </span>
                                                         </td>
                                                      </tr>
                                                   </tbody>
                                                </table>
 ...
 function oportunidades_venta_fugados($id,$fecha){
	global $db, $sugar_config;
  ...
  function mostrar_imgagen_oportunidades($id,$fecha){
	    global $db, $sugar_config;
  ...
  function mostrar_imagen_tareas($id,$fecha){
	  global $db, $sugar_config;
	  	...
	  	$query_tareas_prox_viajes = "SELECT
                      ...
                      FROM tasks ta
						...
						JOIN tasks_cstm ta_c ON ta_c.id_c = ta.id
						JOIN crm_negocios_tasks_c rn ON rn.crm_negocios_taskstasks_idb = ta_c.id_c AND rn.deleted =0
						JOIN crm_negocios neg ON neg.id = rn.crm_negocios_taskscrm_negocios_ida AND neg.deleted =0
						JOIN crm_negocios_crm_pasajeros_c rp ON((rp.crm_negocios_crm_pasajeroscrm_negocios_ida = neg.id)) 
						JOIN crm_pasajeros p ON(((p.id = rp.crm_negocios_crm_pasajeroscrm_pasajeros_idb) 
						AND (p.tipo_pax like '%Comprador%'))) 
						JOIN contacts co ON((co.id = p.contact_id_c))  
                    WHERE ta_c.tipo_c in ('Proximos') 
					     and (neg.fecha_salida >= '".$fecha."') AND (neg.fecha_salida <= ('".$fecha."' + interval 7 day))
	    ...
	    $query_tareas_retorno = "SELECT
                      ...
                      AND date(neg.fecha_destino) = '".$fecha."'
 ...
 function tareas_prox_viajes($id,$fecha){
	global $db, $sugar_config;
 ...
 function tareas_retorno($id,$fecha){
	global $db, $sugar_config;
 ...
 function mostrar_boton_acceder($id,$fecha){
