...
$query = "SELECT acl_roles.* ".
            ...
            while($row = $GLOBALS['db']->fetchByAssoc($result) ){
                ...
                if($row['name'] == 'SUPERVISOR')
                    $supervisor = 1;
                if($row['name'] == 'CONTAC CENTER')
                    $contact_center = 1;
            }
          if($supervisor ==1) $contact_center = 0;  
    ...
    if (isset($old_columns) && !empty($old_columns) && isset($old_dashlets) && !empty($old_dashlets)){
     ...
     ################################################################################   
        ...
        */
        ...
        if($current_user->sucursal_c == 'contact_center' or $contact_center == 1){
                 $defaultDashlets = array(
                        'MyOpportunitiesNuevasDashlet'=>'Opportunities',
                        'MyOpportunitiesPendientesDashlet'=>'Opportunities',
                        'MyOpportunitiesCerradasDashlet'=>'Opportunities',
                      //  'MyOpportunitiesCerradasDashlet'=>'Opportunities',
                         );
        }else{//comparar con rol supervisor, pero como no lo hay 
            
            if($current_user->sucursal_c == 'SUPERVISOR' or $supervisor==1){
            $dashlets[create_guid()] = array('className' => 'iFrameDashlet',
                'module' => 'Home',
                'forceColumn' => 0,
                'fileLocation' => $dashletsFiles['iFrameDashlet']['file'],
                'options' => array('titleLabel' => 'Asignar Oportunidades de Solicitudes Web',
                    'url' => 'http://backend.cochadigital.com/index.php?entryPoint=nuevasoportunidades',
                    'height' => 700,
                )); 

            if($formato_antiguo ==1){
                $dashlets[create_guid()] = array('className' => 'iFrameDashlet',
                    'module' => 'Home',
                    'forceColumn' => 0,
                    'fileLocation' => $dashletsFiles['iFrameDashlet']['file'],
                    'options' => array('titleLabel' => 'Crea Nuevas Oportunidades',
                        'url' => 'http://104.131.0.44/save_opportunity/contactanos.html',
                        'height' => 700,
                    )); 
            }//fin formato antiguo
                $defaultDashlets = array(
