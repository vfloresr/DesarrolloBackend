...
class OpportunitiesHooks{
	...
	function afterSave(&$bean, $event, $arguments=''){
		...
		// Create Task and Call for new Opportunities
		//$this->CreateTaskAndCallForNewOpportunity($bean);
		//'viejo: ".$bean->fetched_row['amount']." 
		//nuevo: ".$bean->amount."'
		// global $db, $sugar_config;
		 // if (isset($bean->fetched_row) && $bean->sales_stage != 'asignado' && ($bean->estado_wf_c == 'notificacion_2h' || $bean->estado_wf_c == 'asignado')){
		//execute logic
				// $oportunidad = new Opportunity();
				// $oportunidad->retrieve($bean->id);
				// $oportunidad->estado_wf_c='cambio_estado';
				// $oportunidad->save();
				//$GLOBALS['log']->fatal("cambio");
		 // }
	}
