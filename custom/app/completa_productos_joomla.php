...
/* if($current_user->user_name!='schiorino' and $current_user->user_name!='admin' and $current_user->user_name!='sbadilla'){
    echo "<script>alert('Funcion habilitada solo para pruebas');window.close()</script>";
 }*/
 ...
 $id_registro = trim($_REQUEST['id_registro']);
  ...
  $tipo_dashlet = trim($_REQUEST['tipo_dashlet']);
   ...
   $id_producto = $oportunidad->aos_products_opportunities_1aos_products_ida;

   $producto = new AOS_products();
   
  $producto->retrieve($id_producto);
  $link_pdf = $producto->link_pdf_c;
  if($producto->origen_c =='joomla')
    $id_producto_joomla = $producto->numero_producto_c;
  else {
  ...
  echo '<div class="container" id="wrap">'."<h2 style='color:red'>Por favor seleccione: ".$producto->name."</h2></div>";
    ...
    $query2 = "SELECT ema.email_address,con.first_name,con.last_name FROM contacts con
        ...
        inner join opportunities_contacts op_c on op_c.contact_id=con.id
        where em_rel.bean_module='Contacts' and con.deleted=0 and op_c.deleted=0
        and op_c.opportunity_id = '".$id_registro."'
        and ema.deleted=0 ;
        ";
       ...
       // $emailtemplate->body_html=str_replace('$account_email1', $correo, $emailtemplate->body_html);
        }
...
<div class="container" id="wrap">
   ...
   ?>
     ...
     <form name="frm3" id="frm3" action="http://104.131.0.44/crm/index.php?entryPoint=envio_email_plantilla" method="POST" enctype="multipart/form-data">
      <div class="row">
...
<!--              <select name="destino" id="destino"  class="form-control col-md-6" >
          ...
          <?php $query2 = "SELECT * from destino "   ;
        
        $res2 = $db->Query($query2);
        while($row2 = $db->fetchByAssoc($res2)){
          $nombre_destino= utf8_decode($row2['nombre']);
          $id_destino= $row2['id'];
     
       ?>
            <option values="<?php echo $nombre_destino?>"><?php echo $nombre_destino?></option>
            <?php }?>
...
fecha de regreso.">Revise documentos viaje</option>
         ...
         <div class="form-group col-md-12" style="display:none">
                <label for="field-1" class="control-label">Telefono Ejecutiva</label>
                  ...
                  <input type="text"  class="form-control" id="telefono_ejecutiva" placeholder="66256354">
  ...
  <tbody>
    ...
    <tr>
      <td><span id="estimado">Estimado</span> <span id="contact_name"><?php echo $nombre_contacto?></span>,
 ...
 <table border="0" cellpadding="1" cellspacing="1" style="width: 30%;">
      ...
      <td style="width: 100px;">
       ...
       <td style=" border-left: 1px solid grey; padding-left:10px">
        ...
        <div id="tejecutiva">$user_cargo</div>
        <div id="tejecutiva">$user_sucursal</div>
        <div id="tejecutiva">$user_telefono</div>
    ...
    $( "#textoemail" ).hide();
        ...
        $( "#incluye" ).click(function() {
         ...
         $( "#contact_name" ).keyup(function() {
          var editor = CKEDITOR.instances.texto;
          if(!editor.document.getById('contact_name'))
              editor.insertText($( "#contact_name" ).val());
          else
              editor.document.getById('contact_name').setHtml($( "#contact_name" ).val()); 
      ...
      $( "#medio_pago" ).change(function() {
        ...
        $( "#telefono_ejecutiva" ).focusout(function() {
          var editor = CKEDITOR.instances.texto;
          editor.document.getById('tejecutiva').setHtml('<strong>'+$( "#telefono_ejecutiva" ).val()+'</strong>'); ; 
          $( "#texto_hide" ).val($( "#texto" ).val());       
        });    
