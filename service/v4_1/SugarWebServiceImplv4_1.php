...
function acceso_user ($parametros){
							$error = new SoapError();
							//authenticate
							global $db, $sugar_config;
							$error = new SoapError();
							//authenticate
							if (!self::$helperObject->checkSessionAndModuleAccess($parametros['session'], 'invalid_session get_cantidad_inicio_nuevos', '', '', '',  $error))
							{
								$GLOBALS['log']->info('End: SugarWebServiceImplv4_1_custom->example_method.');
								return false;
							}
				if ($parametros['modulo'] == 'leads'){
					$leads = new Lead();
					$leads->retrieve($parametros['id']);
						foreach ($parametros as $key => $value){
							if($parametros[$key] != '' && $key != 'modulo' && $key != 'session'){
								$leads->$key = $value;
							}
						}
						$leads->save();
				...
				if ($parametros['modulo'] == 'solicitud'){
					$solicitud = new crm_solicitudes();
					$solicitud->retrieve($parametros['id']);
						foreach ($parametros as $key => $value){
							if($parametros[$key] != '' && $key != 'modulo' && $key != 'session'){
								$solicitud->$key = $value;
							}
				...
				if ($parametros['modulo'] == 'Opportunitys'){
					$Opportunitys = new Opportunity();
					$Opportunitys->retrieve($parametros['id']);
						foreach ($parametros as $key => $value){
							if($parametros[$key] != '' && $key != 'modulo' && $key != 'session'){
								$Opportunitys->$key = $value;
							}
						}
						$Opportunitys->save();
					return $Opportunitys->id;
				}	
				//return false;
	}			
...
//evento utilizado para detectar la actividad del usuario
				...
				//------------------------------------------------------
					 ...
					 select distinct o.id AS id_oportunidad,
									...
									CONVERT_TZ(`do`.`date_assigned_c`,'UTC','America/Santiago') AS fecha_asignacion_c,
									...
									and o.sales_stage in ('asignado','AsignacionSolicitudSupervisor','RecepcionSolicitud')
...
// creado por armando 02/11/2015				
  ...
  function set_editar_cumple_auto($session,$usuario_asig,$delete,$html,$asunto,$comentarios,$campos){
		global $db, $sugar_config;
		$GLOBALS['log']->info('Begin: SugarWebServiceImplv4_1_custom->example_method');
				$error = new SoapError();
				//authenticate
				...
				//------------------------------------------------------
				//en esta ruta se encuentran los posiblea campos a utiilizar (C:\xampp\htdocs\prototipo\modules\Contacts)
				//return $session.','.$usuario_asig.','.$delete.','.$html.','.$asunto.','.$comentarios.','.$campos;
			$contacto = new Contact();
			$user= new User();
			...
			//return $campos;
			// $email = new Email();
			// $email->retrieve('addcc16e-41ce-f78c-5230-57168a2509fa');
			// return $email;
			if ($delete == '1'){
				foreach ($campos as $value) {
					...
					$query4=" SELECT id
						FROM
						  emails e 
						WHERE
						e.parent_type = 'contacts'
						and SUBSTRING(e.name,1,11) =  'CUMPLEANIOS'
						and e.deleted = '0'
						and year(e.date_sent) = year(now())
						and  e.parent_id = '".$value."'";
					 $res3 = $db->Query($query4);
					if($row3 = $db->fetchByAssoc($res3)){
						$email = new Email();
						$email->retrieve($row3['id']);
						$email->deleted = 1;
						$email->save();
						if($email->id){
							$respuesta=1;
						...
						echo '<script>alert("El Cumpleañero no puede Modificado ya que no ha sido Asignado Todavia");</script>';
						return 0;
					}
				}
			}else{			
				foreach ($campos as $value) {
					$contacto->retrieve($value);
					if($contacto->id != null){
						$plantilla = $html;
						// Parse Body HTML
						$color = "#585858";
						$plantilla= str_replace('$user_fullname', ucwords($user->full_name), $plantilla);
						$plantilla= str_replace('$email', $user->email1, $plantilla);
						$plantilla= str_replace('$telefono', $user->phone_work, $plantilla);
						$plantilla= str_replace('$sucursal', $user->department, $plantilla);
						$plantilla= str_replace('$contact_name', ucwords($contacto->full_name), $plantilla);
						//$plantilla= str_replace('<span style="color:#696969;font-family: arial, sans-serif;font-size: 12.8000001907349px;line-height: normal;" class="mail_modal_comentario_value"></span>', $comentarios, $plantilla);
						$plantilla= str_replace('<span style="color:#585858;font-family: arial, sans-serif;font-size: 12.8000001907349px;line-height: normal;" class="mail_modal_comentario_value"></span>', "<p style='color:black;'><font color='".$color."'>".$comentarios."</font></p>", $plantilla);
						$email = new Email();                                   
						$email->assigned_user_id=$user->id;
						$envio = explode("-",$contacto->birthdate);
						$email->date_sent=date('Y-m-d', strtotime($envio[2].'-'.$envio[1].'-'.date("Y")));
						$email->name = 'CUMPLEANIOS:'.$asunto;
						$email->type='out';
						$email->status='AGENDADO';
						$email->parent_type='contacts';
						$email->parent_id=$contacto->id;
						$email->to_addrs= $contacto->email1;
						$email->cc_addrs= $user->email1;
						$email->from_addr = $user->email1;
						$email->intent='pick';
						$email->description_html=$plantilla;
						$email->assigned_user_name=$user->user_name;
	...
	function get_cumple_mes($session,$usuario_asig){
			...
			//$cantidad_cumpleanos = count($cumpleanos_list);
			 ...
			 foreach ($list_task as $n) {
				...
				$query4=" SELECT  count(*) as notificacion_c
						FROM
						  emails e 
						WHERE
					    e.parent_type = 'contacts'
				 ...
				 $res3 = $db->Query($query4);
				if($row3 = $db->fetchByAssoc($res3)){
					$obj ['notificacion_c'] = $row3['notificacion_c'];
				}
				$contacto =new Contact();
				...
				// caso de notificacion email
				$query3="SELECT 
						  e.parent_id,
						 GROUP_CONCAT(distinct IF(SUBSTRING(e.name,1,11) = 'CUMPLEANIOS','a','m')) as estado
						FROM
						  emails e 
						WHERE
							e.parent_type = 'contacts' 
			...
			$error = new SoapError();
				...
				//------------------------------------------------------
							$query="
									...
									JOIN (SELECT pa.contact_id_c as rut,
												n.name as negocio,
										   pa.tipo_pax,
...
// creado por armando 02/11/2015
				...
				$query="SELECT 
						...
						while($value = $db->fetchByAssoc($respt)){
							...
							$obj ['description']= $value['description'];
								...
								$obj ['p']= $oppor_o;
								$query_3="SELECT  negocio,tipo,rol,destino,ruta,f_salida,f_destino,id,monto_venta,comprador,pasajeros
										  FROM view_tareas_rut  
										  WHERE pasajeros = '".$value['rut']."'
											AND usuario_asig = '".$usuario_asig."'
											AND deleted = '0'";
								$respt_3 = $db->Query($query_3);
	...
	// creado por armando 02/11/2015
	/*este procceso edita la informacion del estado de la oportunidad*/
	function set_editar_oportunidad_web($session,$usuario_asig,$oportunidad_id,$campos_opor){
...
// creado por armando 02/11/2015
 ...
 //return $session.'_'.$usuario_asig.'_'.$correo.'_'.$asunto.'_'.$email_cc.'_'.$name_file_array.'_'.$contacto_id.'_'.$ruta_upload.'_'.$from.'_'.$nombre_user.'_'.$procedencia.'_'.$procedencia_id.'_'.$pdf.'_'.$estado;
		...
		//$GLOBALS['log']->fatal("\n"."INPUT"."\n"."session:".$session."\n"."Usuario:".$usuario_asig."\n"."correo CLIENTE:".$correo.'\n'."ASUNTO:".$asunto."\n"."email:".$email_cc."\n"."name-file:".$name_file_array."\n"."contacto:".$contacto_id."\n"."ruta:".$ruta_upload."\n"."from:".$from."\n"."nombre user:".$nombre_user."\n"."procedencia:".$procedencia."\n"."procedencia id:".$procedencia_id."\n"."pdf:".$pdf.'\n'."estado:".$estado."\n");
		...
		$this->actividad_user($usuario_asig);
				//------------------------------------------------------
		//die();
		if (($contacto_id != '') || ($contacto_id != null)){
				//return "contacto WS:".$contacto_id."<br>";
			$contacto = new Lead();
		//	$contacto->retrieve_by_string_fields(array('phone_fax' => $contacto_id));
			$contacto->retrieve($contacto_id); 
			//return "contacto WS:".$contacto_id."<br>";
			//return "contacto WS:".$contacto->retrieve($contacto_id)."<br>";
			//return $contacto->retrieve($contacto_id);
			
			//$GLOBALS['log']->fatal("\n"."OUTPUT"."\n"."Contacto ID:".$contacto->retrieve($contacto_id)."\n"."Contacto Salida:".$contacto->first_name."\n"."Apellido:".$contacto->last_name."Email:".$contacto->email1."\n");
			
			//return("\n"."OUTPUT"."\n"."Contacto ID:".$contacto->retrieve($contacto_id)."\n"."Contacto Salida:".$contacto->first_name."\n"."Apellido:".$contacto->last_name."Email:".$contacto->email1."\n");
							
		}else{
				return 'Error, contacto o email no existe '.$contacto_id;
		}
					...
					//return "mail->From:".$mail->From."<br>";//llega bien
		            //die();
					$mail->FromName = $nombre_user;
					//return "FromName:".$mail->FromName."<br>";//llega bien
		            //die();
					...
					$mail->prepForOutbound();
					 if($contacto_id != ''){
							$mail->AddAddress($contacto->email1);
							//$mail->AddAddress($correo);
							//$mail->AddAddress("santa.rason@gmail.com");
				     //return "Mail AddAddress:".$mail->AddAddress($contacto->email1)."<br>";//llega vacio
		            //die();	
					 }else{
							$mail->AddAddress($correo);
						//return "Mail AddAddress Correo:".$mail->AddAddress($correo)."<br>";//llega vacio
		                //die();	
					}
					///$GLOBALS['log']->fatal("EMAIL:".$contacto->email1."\n");
					 #########################adjunta documento##################################
					...
					$update= "UPDATE opportunities opp
								SET    opp.assigned_user_id = opp.modified_user_id
								WHERE  opp.assigned_user_id = '1' AND opp.sales_stage IN ('PrimeraLlamada', 'EnvioCotizacion', 'Seguimiento1', 'Seguimiento2',
									   'AsignacionSolicitudSupervisor', 'contactado', 'cotizacion', 'negociacion')";
					$resul = $db->Query($update);
	...
	function get_oportunidad_detalle_producto_web($session,$usuario_asig,$oportunidad_id){
			...
			while($value = $db->fetchByAssoc($respt)){	
				...
				$usuario->retrieve($value['id_user']);
				  ...
				  $detalle['operador'] = $value['tour_operador'];
	...
	function set_aprueba_solicitud($session,$usuario_asig,$campos){
		...
		//evento utilizado para detectar la actividad del usuario
			...
			if ($campos['respuesta'] == '1'){
				...
				$tarea->description=TimeDate::getInstance()->getNow(true)->modify("-4 hours")->asDb().'/['.date('Y-m-d H:i:s').'] Solicitud aceptada por el usuario: '.$campos['user_name'];
				$tarea->save();
				...
				while($value2 = $db->fetchByAssoc($respt_2)){
					...
					$oportunidad->date_assigned_c = TimeDate::getInstance()->getNow(true)->asDb();
					$oportunidad->save();
					$i=$i+1;
				}
			...
			/*	if(@$mail->Send()){
				...
				$tarea->status='Completed';
				$tarea->description=$row['description'].'/['.TimeDate::getInstance()->getNow(true)->modify("-4 hours")->asDb().'] Solicitud rechazada por el usuario: '.$campos['user_name'];
				$tarea->save();
